Vue.component('partial', {
    template: '<div>' +
        ...
        '<span class="optionIcon" v-if="(index+1)==batchDevice.length&&item.list.length>1&&isDetail" @click="addDeviceSwitch(item.list,index)">' +
        ' <i class="el-icon-circle-plus-outline"></i></span>' +
        '<span class="optionIcon" v-if="(index+1)<batchDevice.length&&isDetail" @click="editDeviceSwitch(item,index)"> <i class="el-icon-edit"></i></span>' +
        '<span class="optionIcon" v-if="(index+1)<batchDevice.length&&isDetail" @click="delDeviceSwitch(item.list,index)"><i class="el-icon-delete"></i></span>' +
        '</p>' +
        ...
        '<p><span>{{i18ns.upgradePlan.upgradePlan_delayTime}}：</span> <el-input-number v-model="delayTime" :precision="1" :step="0.1" :min="0" :max="24"></el-input-number></p>' +
