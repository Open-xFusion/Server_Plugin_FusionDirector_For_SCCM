Vue.component("device-version", {
    template: ' <div>' +
        ...
        '<el-table-column v-for="(item,index) in columns" :prop="item.prop" :label="i18ns.deviceVersionStatus[item.label]" :key="index" :show-overflow-tooltip="true">'+
            '<template slot-scope="scope">'+
                '<div v-if="item.iconClassAfter===true">'+
                    '{{scope.row.AvailableVersion}}<img src="../../images/upstatus.png" style="width: 12px; height:12px; margin-left: 5px;" v-if="isup(scope.row)" />'+
                    '<img src="../../images/downstatus.png" style="width: 12px; height:12px; margin-left: 5px;" v-if="isdown(scope.row)" />'+
                '</div>'+
                '<div v-else>'+
                    '{{scope.row[item.prop]}}'+
                '</div>'+
    ...
    methods: {
        isup: function (row){
            if (this.checkDataIsValid(row.AvailableVersion)) {
                if(row.AvailableVersion > row.InstalledVersion){
                    return true
                }
            }
            return false
